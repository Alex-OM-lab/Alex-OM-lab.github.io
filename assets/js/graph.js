...
const SiteGraph = (() => {
  ...
  function drawNode({level,x,y,w,h,title,sub,onClick,delay=0}){
    ...
    // grupo interno para micro-slide
  ...
  function drawLink(from,to,level,delay=0){
    ...
    const dx = Math.max(60, (x2-x1)/2); // más espacio → curva más elegante
  ...
  function highlightActive(){
    ...
    if(G.state.sel0!=null){
      const n0 = G.colsNodes[0][G.state.sel0]; n0?.el.classList.add('active');
    }
    // nivel 1
    if(G.state.sel1!=null){
      const n1 = G.colsNodes[1][G.state.sel1]; n1?.el.classList.add('active');
      n1?._link?.classList.add('active');
    }
    // nivel 2
    if(G.state.sel2!=null){
      const n2 = G.colsNodes[2][G.state.sel2]; n2?.el.classList.add('active');
      n2?._link?.classList.add('active');
    }
  }
